...
function renderViewChannel() {
	...
	if (addedChannelData.length > 0) {
		addedChannelData.forEach((item, index) => {
			...
			str2 += `
					<div class="add-channel-list" data-id="${item.event_id}">
	...
	$('.channel-cont-add').dad({
		draggable: '.add-channel-list-info',
		callback: function () {
			moveChannelEnd()
		}
	})
...
function moveChannelEnd(){
	var data = []
	$('.add-channel-list').each((index,dom)=>{
		
		addedChannelData.forEach(item=>{
			if(Number($(dom).attr('data-id'))===Number(item.event_id)){
				data.push(item)
			}
		})
	...
	$('.wonderful-channel-cont').html(str1)
	addedChannelData=JSON.parse(JSON.stringify(data))
}
...
function renderViewVideo() {
	...
	if (addedVideoData.length > 0) {
		addedVideoData.forEach((item, index) => {
			...
			str2 += `
						<div class="add-video-list" data-id="${item.video_id}">
	...
	$('.video-cont-add').dad({
		draggable: '.add-video-list-info',
		callback: function () {
			moveVideoEnd()
		}
	})
}
function moveVideoEnd(){
	var data = []
	$('.add-video-list').each((index,dom)=>{
	...
	$('.wonderful-video').html(str1)
	addedVideoData=JSON.parse(JSON.stringify(data))
}
